...
class Game{
     ...
     */
    getState(){
        return this.state.toString().match(/\((.*)\)/)[1].toLowerCase();
    }
    ...
    newBoard(){
        ...
        for(let i = 0; i < this._board_size;i++){
            this.board.push(new Column(this._board_size,i));
        }
    }
...
Game.moving = Symbol("Moving");

// End phase - the game is over
Game.end = Symbol("End");