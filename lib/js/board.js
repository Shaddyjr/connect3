...
class Board{
    ...
    /**
     * Creates a static testing board for debugging purposes.
     */
    createTestBoard(){
        this._n = 5;
        this.resetBoard();
        this.board[0].column.forEach(x=>x.gem = Gem.STAR);
        
        this.board[1].column.forEach(x=>x.gem = Gem.STAR);

        this.board[2].column[0].gem = Gem.SQUARE;
        this.board[2].column[1].gem = Gem.SQUARE;
        this.board[2].column[2].gem = Gem.SQUARE;
        this.board[2].column[3].gem = Gem.SQUARE;
        this.board[2].column[4].gem = Gem.CIRCLE;
        
        this.board[3].column[0].gem = Gem.SQUARE;
        this.board[4].column[0].gem = Gem.SQUARE;
        this.board[4].column[1].gem = Gem.SQUARE;
        this.board[4].column[2].gem = Gem.SQUARE;
        this.board[4].column[3].gem = Gem.SQUARE;
        this.board[4].column[4].gem = Gem.CIRCLE;

        this.board[3].column[1].gem = Gem.TRIANGLE;
        this.board[3].column[2].gem = Gem.TRIANGLE;
        this.board[3].column[3].gem = Gem.TRIANGLE;

        this.board[3].column[4].gem = Gem.CIRCLE;
    }
    ...
    /**
     * Returns an array of the prunned match 3+ gem groups on the board.
     */
    getGemGroups(){
        let output = [];
        ...
        for(let col_i = 0; col_i < this._n; col_i++){
            ...
            for(let row_i = 0; row_i < this._n; row_i++){
                ...
                const gemBlob = this._createGemBlob(gem);
                if(gemBlob.hasValidGroupings()) output.push(gemBlob);
            }
        }
        return output.reduce((acc,gemBlob)=>acc=acc.concat(gemBlob.extractGemGroups()),[]);
    }

    /**
     * Returns an array of the given gem's match 3+ group blob.
     */
    _createGemBlob(gem, output){
        output = output || [];
        output.push(gem);
        gem.visited = true;
        ...
        for(const neighbor in gem.neighbors){
            ...
            if(coords){
                ...
                if(!neighbor_gem.visited && neighbor_gem.gem === gem.gem){
                    this._createGemBlob(neighbor_gem, output);
                }
            }
        }
    ...
    /**
     * Removes matche 3+ groups currently on the board until none are left.
     */
    clearMatches(){
        this.gemGroupQueue = this.getGemGroups();
        while(this.getState() === Board.PROCESSING){
            const gemGroup = this.gemGroupQueue.pop();
            if(gemGroup.centerCoord){
                const col = gemGroup.centerCoord[0];
                const row = gemGroup.centerCoord[1];
                this.board[col].column[row].gem = Gem.randomBaseGem(this.board[col].column[row].gem.gem);
            }else{
                for(const coords of gemGroup.gemCoords){
                    const col = coords[0];
                    const row = coords[1];
                    this.board[col].column[row].gem = Gem.randomBaseGem();
                }
            }
            this.gemGroupQueue = this.getGemGroups();
        }
     ...
     */
    getState(){
        return this.gemGroupQueue.length ? Board.PROCESSING : Board.READY;
    }
}
Board.PROCESSING = Symbol("processing");
Board.READY      = Symbol("ready");
